...
source=nbformat.read(ROOT/'notebooks/normative_hysteresis_objective_transfer_colab.ipynb',as_version=4)
...
work=ROOT/'.context/validation'/('objective-transfer-'+uuid.uuid4().hex[:8]);work.mkdir(parents=True)
...
extracted=work/('nh-objective-transfer-src-'+source.metadata.source_bundle_sha256[:12])
for name in ['objective_transfer.py','configs/objective_transfer_v1.json','transfer_tests/test_transfer.py']:
...
with zipfile.ZipFile(archives[0]) as z:
    ...
    assert len([n for n in z.namelist() if '/records/' in n])==320
    assert 'source/objective_transfer.py' in z.namelist()
    assert any(n.endswith('/design_audit.json') for n in z.namelist())
print('PASS: portable sources, offline tests, 320 saved synthetic failures, completed resume without generation, analysis and export.')
