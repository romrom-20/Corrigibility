#!/usr/bin/env python3
"""Build the standalone calibration notebook; does not regenerate v2."""
import base64
import hashlib
import json
...
def build():
    paths = [ROOT/'calibration_bench.py', ROOT/'configs/calibration_v1.json', ROOT/'requirements-colab.txt',
             *sorted((ROOT/'corrigibility_bench').glob('*.py')),
             *sorted((ROOT/'calibration_tests').glob('*.py')),
             ROOT/'docs/CALIBRATION_PLAN.md', ROOT/'docs/NEXT_AGENT_PROMPT.md',
             ROOT/'docs/history/DIAGNOSTIC_V2_20260915.md']
...
PROJECT_ROOT = (Path('/content') if Path('/content').exists() else Path.cwd()) / ('nh-calibration-src-' + BUNDLE_SHA256[:12])
for name in ('calibration_bench', 'corrigibility_bench'):
...
'''.replace('__FINGERPRINT__',sha).replace('__ENCODED__',encoded)
    cells = [cell('markdown','''# Isolated selection and probe calibration

This is **nh-calibration-v1**, not a hysteresis pilot. It makes **272 independent calls**:
192 selection calls and 80 probe calls. Two samples per cell. No automatic scaling,
no previous response fed into another call, and no retries for wrong/invalid answers.
Read `docs/CALIBRATION_PLAN.md` after extraction. Use hosted Colab only; never a local runtime.
...
Each call verifies effective decoding. Selection cap is 512 in all four formats;
probe cap is 160 in both formats. A changed source/config/runtime requires a new ID.
...
'''), cell('code',"from corrigibility_bench.hf_backend import HFBackend\nbackend = HFBackend(config)\nprint(json.dumps(backend.metadata, indent=2))"),
        cell('markdown','''## Run or resume calibration

The default new run ID is `calibration-001`. Completed matching calls are reused.
...
There is no smoke-review gate here: these calls are diagnostic calibration only.
'''), cell('code',"RUN_ID = 'calibration-001'\nrun_dir = cal.run(backend, config, RESULTS_ROOT, RUN_ID)\nprint('Saved:', run_dir)"),
        cell('markdown','''## Inspect and export

Review summary.json, cases.csv, design_audit.json and all transcripts. Compare
shortlist versus shortlist_reworded first (one sentence differs); then rowwise
eligibility errors and ranking_only success. Compare probe components by context.
The ranking-only condition uses an oracle filter: success does not prove the model
can perform the complete task. See the plan for interpretation and next decisions.
