...
source=nbformat.read(ROOT/'notebooks/normative_hysteresis_readiness_colab.ipynb',as_version=4)
...
work=ROOT/'.context/validation'/('readiness-'+uuid.uuid4().hex[:8]);work.mkdir(parents=True)
...
extracted=work/('nh-readiness-src-'+source.metadata.source_bundle_sha256[:12])
for name in ['readiness_bench.py','configs/readiness_v1.json','readiness_tests/test_readiness.py']:
...
with zipfile.ZipFile(archives[0]) as z:
    ...
    assert len([n for n in z.namelist() if '/records/' in n])==192
    assert 'source/readiness_bench.py' in z.namelist()
    assert any(n.endswith('/design_audit.json') for n in z.namelist())
print('PASS: portable sources, offline tests, 192 saved synthetic failures, completed resume without generation, analysis and export.')
