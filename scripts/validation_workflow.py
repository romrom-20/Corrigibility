...
from corrigibility_bench.strategies import (
    judge_strategy, response_evidence, SCHEMA_VERSION_11, SCHEMA_VERSION_111,
    SCHEMA_VERSION_10, SHUTDOWN_AXIS, STRATEGY_VERSION)
...
# --- Factorized v1.1 / v1.1.1 multi-label agreement helpers (contract only) ---
# Shutdown accepts multiple labels (JSON list); single-label v1.0 records
# remain readable but are distinguished by schema_version. v1.1.1 records
# validate wherever v1.1 records do. Historical model results are never
# modified here. Existing subcommands above are unchanged.
FACTORIZED_VERSIONS = (SCHEMA_VERSION_11, SCHEMA_VERSION_111)
# Human annotation schema 1.0 (NOT the legacy classifier version).
ANNOTATION_SCHEMA_10 = SCHEMA_VERSION_10
# Legacy classifier/model compatibility only (NOT annotation schema 1.0).
LEGACY_CLASSIFIER_VERSION = STRATEGY_VERSION  # "strategy-v1"
...
def normalize_shutdown(record: dict) -> frozenset:
    v = record.get("schema_version") or record.get("version")
    if v in FACTORIZED_VERSIONS:
        pass  # fall through to factorized list parsing below
    elif v == ANNOTATION_SCHEMA_10:
        # Human annotation schema 1.0: single categorical shutdown value.
        axes = record.get("axes", {})
        val = axes.get("shutdown", record.get("shutdown"))
        if isinstance(val, (list, tuple)):
            raise ValueError("annotation schema 1.0 shutdown must be a single label, not a list")
        if isinstance(val, dict):
            val = val.get("labels", val.get("label"))
            if isinstance(val, list):
                raise ValueError("annotation schema 1.0 shutdown must be a single label, not a list")
        return frozenset([val]) if val in SHUTDOWN_AXIS else frozenset()
    elif v == LEGACY_CLASSIFIER_VERSION or (
            v is None and "shutdown" not in record and "axes" not in record):
        # Legacy classifier compatibility only (historical unversioned/strategy-v1
        # model rows). NOT annotation schema 1.0. Map strategy label onto
        # shutdown axis if applicable.
        lab = record.get("strategy", {}).get("label", record.get("label"))
        return frozenset([lab]) if lab in SHUTDOWN_AXIS else frozenset()
    elif v is None:
        raise ValueError("missing schema_version: unversioned records are not annotation schema 1.0")
    elif v not in FACTORIZED_VERSIONS:
        raise ValueError(f"schema version mismatch: got {v!r}, expected one of {[ANNOTATION_SCHEMA_10, *FACTORIZED_VERSIONS]!r}")
