...
def build():
    paths = [ROOT/'objective_transfer.py', ROOT/'readiness_bench.py', ROOT/'calibration_bench.py', ROOT/'configs/objective_transfer_v1.json', ROOT/'requirements-colab.txt',
             *sorted((ROOT/'corrigibility_bench').glob('*.py')),
             *sorted((ROOT/'transfer_tests').glob('*.py')),
...
PROJECT_ROOT = (Path('/content') if Path('/content').exists() else Path.cwd()) / ('nh-objective-transfer-src-' + BUNDLE_SHA256[:12])
for name in ('objective_transfer', 'readiness_bench', 'calibration_bench', 'corrigibility_bench'):
...
'''.replace('__FINGERPRINT__',sha).replace('__ENCODED__',encoded)
    cells = [cell('markdown','''# Objective transition feasibility

This is **nh-objective-transfer-v1**, not a hysteresis pilot. It makes **128 trajectories / 320 calls**: 64 planning calls and 256 independent terminal
siblings. All items, row orders and label variants are retained across four controls
and k=0/1. Planning responses are retained verbatim; terminal siblings never see
each other. No automatic scaling or retries for wrong/invalid answers.
Factual specificity is unresolved: failed factual controls are not included here.
Read `docs/OBJECTIVE_TRANSFER_PLAN.md` after extraction. Use hosted Colab only; never a local runtime.
...
'''), cell('code',bootstrap), cell('markdown','## Install Colab dependencies'),
        ...
        cell('code',"import unittest\nresult = unittest.TextTestRunner(verbosity=2).run(unittest.defaultTestLoader.discover('transfer_tests'))\nassert result.wasSuccessful() and not result.skipped\nimport objective_transfer as cal\nconfig = cal.load_config()\nprint('Budget:', cal.budget())\nprint(json.dumps(cal.ready.design_audit(), indent=2))"),
...
The default new run ID is `objective-transfer-001`. Completed matching calls are reused.
...
This is a bounded follow-up to the reviewed readiness results, not approval of a
larger experiment. Preserve all responses and inspect the full result before any expansion.
'''), cell('code',"RUN_ID = 'objective-transfer-001'\nrun_dir = cal.run(backend, config, RESULTS_ROOT, RUN_ID)\nprint('Saved:', run_dir)"),
        cell('markdown','''## Inspect and export

Review summary.json, trials.csv, design_audit.json and every transcript. Check the
